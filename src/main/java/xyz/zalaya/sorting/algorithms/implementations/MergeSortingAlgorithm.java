...
public class MergeSortingAlgorithm implements SortingAlgorithm {
    ...
    public <T extends Comparable<T>> T[] sort(T[] unsortedArray) {
        recursiveMergeSortSubArrays(unsortedArray, 0, unsortedArray.length - 1);

        return unsortedArray;
    }

    private <T extends Comparable<T>> void recursiveMergeSortSubArrays(T[] unsortedArray, int startIndex, int endIndex) {
        if (startIndex < endIndex) {
            int middleIndex = (startIndex + endIndex) / 2;

            recursiveMergeSortSubArrays(unsortedArray, startIndex, middleIndex);
            recursiveMergeSortSubArrays(unsortedArray, middleIndex + 1, endIndex);

            mergeSubArrays(unsortedArray, startIndex, middleIndex, endIndex);
        }
    }

    private <T extends Comparable<T>> void mergeSubArrays(T[] mergedArray, int startIndex, int middleIndex, int endIndex) {
